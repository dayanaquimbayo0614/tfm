...
def train_model(X_train,y_train, model, name_model):
    t_0 = time.time()
    joblib.dump(model.fit(X_train, y_train),name_model)
    t_1 = time.time()
    print('Tiempo de ejecución: ',(t_1 - t_0)/60)
    
    
def eval_model(X_test, y_test, saved_model, name_model, dic_metrics):
